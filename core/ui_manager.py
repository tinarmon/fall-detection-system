...
class UIManager:
    ...
    def draw_hud(self, frame, tester_name, fps, status_text, prediction, theme_color, bbox):
        ...
        cv2.putText(frame, f"TESTER : {tester_name}", (30, 45),  self.font, 0.6, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(frame, f"FPS    : {int(fps)}",    (30, 75),  self.font, 0.6, (255, 255, 255), 1, cv2.LINE_AA)
        cv2.putText(frame, f"STATUS : {status_text}", (30, 108), self.font, 0.7, theme_color,      2, cv2.LINE_AA)
        ...
        cv2.putText(frame, f"{float(prediction) * 100:.0f}%",
                    (bar_x + bar_w + 8, bar_y + 10), self.font, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
